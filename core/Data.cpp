...
{
	const auto& firstPointOf2ndRowItr = std::find_if(_orientation.begin(), _orientation.end(),
																						[&](const auto& elem_) {return elem_[coordinates::y] > ZERO_DOUBLE;});
	
	CUSTOM_ASSERT(firstPointOf2ndRowItr != _orientation.end(), "Couldn't find non-zero y coordinate in data");

	_numOfPointsInTheBottomRow = std::distance(std::prev(firstPointOf2ndRowItr), _orientation.begin()) + 1; //including the last element
